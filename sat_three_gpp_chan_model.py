...
class sat_three_gpp_channel_model(object):
    ...
    def step4(self):
        ...
        correlation_matrix, spread_matrix = th_gpp_T.get_correlation_spread(self.scenario, self.link_state, self.elev_angle_ref)
        shadow_CL_table = th_gpp_T.get_shadowing_params(self.scenario, self.elev_angle_ref)
    ...
    def step5_and_step6(self):    
        ...
        cluster_shadowing_std= th_gpp_T.get_per_cluster_shadowing_std(self.scenario, self.link_state, self.elev_angle_ref)
        # step 5
        self.tau_n_list = []
        for i,link_state_i in enumerate(self.link_state):
    ...
    def step7_step8_step9(self):
        ...
        for i, link_state_i in enumerate(self.link_state):
            ...
            phi_clusters_AOA_prime = 2*(self.ASA_list[i]/1.4)*np.sqrt(-np.log(P_clusters/max(P_clusters)))/C_phi
            phi_clusters_AOD_prime = 2*(self.ASD_list[i]/1.4)*np.sqrt(-np.log(P_clusters/max(P_clusters)))/C_phi
            ...
            phi_AOA_cluster_rays = phi_AOA_clusters + C_ASA_list[i]*np.array(ray_offset_angles)[:,None] # (n_ray, n_cluster)
            phi_AOD_cluster_rays = phi_AOD_clusters + C_ASD_list[i]*np.array(ray_offset_angles)[:,None] #(n_ray, n_cluster)
            ...
            theta_ZOA_cluster_rays = theta_ZOA_clusters + c_ZSA* np.array(ray_offset_angles)[:,None] # (n_ray, n_cluster)
            ...
            # 38.901, 7.5-20
            theta_ZOD_cluster_rays = theta_ZOD_clusters + (3/8)*(10**(mu_lg_ZSD))*np.array(ray_offset_angles)[:,None] #(n_ray, n_cluster)
           ...
           for i in range (len(self.link_state)): 
               ...
               for n in range(n_clusters):
                   ...
                   for k in range(n_rays):
                       ...
                       coupling_M = np.array([[np.exp(1j*TT[k,n]),np.sqrt(1/kappa[k,n])*np.exp(1j*TP[k,n])],
                       ...
                       # shape =  (n_ray, n_cluster)
                       H_nlos[k,n] = np.sqrt(P_n/n_rays)*F_ue.T.dot(coupling_M).dot(F_sat)\
                           *np.exp(1j*2*np.pi*np.dot(r_tx, d_tx)/lambda_0)\
                           *np.exp(1j*2*np.pi*np.dot(r_rx, d_rx)/lambda_0)\
                           *np.exp(1j*2*np.pi*np.dot(r_rx, self.r_velocity[i])*self.time/lambda_0)
               ...
               if self.link_state[i] ==1:
                   ...
                   # set all input angles as radians
                  ...
                  # print(sat_ant_gain_los_list[i],i)
                   ...
                   # 38.901, 7.5-29
                   H_los = F_ue.T.dot(coupling_M).dot(F_sat)\
                   *np.exp(-1j*2*np.pi*dist3D/lambda_0)\
                   *np.exp(1j*2*np.pi*np.dot(r_tx, d_tx)/lambda_0)\
                   *np.exp(1j*2*np.pi*np.dot(r_rx, d_rx)/lambda_0)\
                   *np.exp(1j*2*np.pi*np.dot(r_rx, self.r_velocity[i])*self.time/lambda_0)
                   
                   H_los = np.squeeze(H_los)
