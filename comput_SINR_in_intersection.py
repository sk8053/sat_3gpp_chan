...
theta = 0*np.pi/180 # [rad] longtitude, 0: equator 
n_ue_per_cell = 1 # number of UE per cell
...
y = 2*np.pi*R_E/N_s # vertical length between satellites
...
# adjacent beamwidth
ABS = np.sqrt(3) *np.sin(np.deg2rad(HPBW)/2)*sate_height
# beam radius
beam_radius = ABS/(2*np.cos(np.pi/6))
# number of tier to cover the whole area of footprint 
n_tier = np.ceil(R_footprint/ABS) +1 
n_tier = int(n_tier)
...
########################################################################################################
############################## SINR Computation for Opne Loop MIMO ########################################################
# create random beamforming vector across frequency domain
...
for iter_ in range(1):
    for link_idx in tqdm(range(ue_inter_sec_new.shape[0])):
        ...
        for f_idx in range(len(freq_list)):
            ...
            for beam_idx in range(len(ind_in_intersec)):
            ...
            for beam_idx in serving_ind:
                if beam_idx != serv_beam_idx:
                    # create interference channel shape = (2, 2)
                    h_itf_45_1 = H_45_dict[0][link_idx,beam_idx,f_idx] 
                    h_itf_n_45_1 = H_n_45_dict[0][link_idx,beam_idx,f_idx]
                    h_itf_45_2 = H_45_dict[1][link_idx,beam_idx,f_idx] 
                    h_itf_n_45_2 = H_n_45_dict[1][link_idx,beam_idx,f_idx]
...
plt.figure()
plt.plot(np.sort(SINR_avg_list), np.linspace(0,1,len(SINR_avg_list)),'r', label = 'with precoder cycling')
#plt.plot(np.sort(SINR_avg_list_no_precoder), np.linspace(0,1,len(SINR_avg_list)),'k', label = 'without precoder cycling')
plt.legend()
plt.grid()